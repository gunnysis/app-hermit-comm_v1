...
- **감정 분석 자동 호출**: 마이그레이션 016부터 DB 트리거는 제거됨. **Supabase Dashboard → Database → Webhooks**에서 **posts / Insert** → `analyze-post` Webhook을 반드시 추가해야 새 글 작성 시 감정 분석이 호출됨. Edge Functions 배포·시크릿(`GEMINI_API_KEY`)은 `supabase/functions/CONSOLE_SETUP.md` 참고.
...
- **Edge Functions·감정 분석**: 배포는 `supabase functions deploy analyze-post` 등. `GEMINI_API_KEY` 시크릿 및 **Database Webhook(posts INSERT → analyze-post)** 설정은 `supabase/functions/CONSOLE_SETUP.md` 참고.
